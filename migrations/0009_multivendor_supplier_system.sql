-- Migration: Multivendor Supplier System
-- Description: Add comprehensive supplier system with profiles, commission management, payouts, reviews, and staff management
...
ALTER TABLE orders ADD COLUMN supplier_id VARCHAR REFERENCES supplier_profiles(id) ON DELETE SET NULL;
...
CREATE INDEX idx_supplier_profiles_store_slug ON supplier_profiles(store_slug);
CREATE INDEX idx_supplier_profiles_status ON supplier_profiles(status);
CREATE INDEX idx_supplier_profiles_membership_tier ON supplier_profiles(membership_tier);
CREATE INDEX idx_supplier_profiles_verification_level ON supplier_profiles(verification_level);
CREATE INDEX idx_supplier_profiles_is_active ON supplier_profiles(is_active);
CREATE INDEX idx_supplier_profiles_rating ON supplier_profiles(rating DESC);
CREATE INDEX idx_supplier_profiles_total_sales ON supplier_profiles(total_sales DESC);
CREATE INDEX idx_supplier_profiles_created_at ON supplier_profiles(created_at DESC);
...
CREATE INDEX idx_products_supplier_approved ON products(supplier_id, is_approved);
...
-- Update users table role to include supplier
...
CREATE TRIGGER trigger_update_supplier_product_count
  ...
  FOR EACH ROW
  WHEN (NEW.supplier_id IS NOT NULL OR OLD.supplier_id IS NOT NULL)
